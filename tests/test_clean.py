...
def check_frequency_of_occurance(series, column_name, min_frequency_of_occurrence, include_other_category, map_rare_to_other_threshold):
    ...
    if len(item_frequencies) > 0:
        ...
        if include_other_category:
            ...
            assert least_common_frequency >= map_rare_to_other_threshold, f"Error in frequencies of {column_name}"
        else:
            # If 'other' is not included, then the least common item must appear at least `min_frequency_of_occurrence` times
            assert least_common_frequency >= min_frequency_of_occurrence, f"Error in frequencies of {column_name}"
    else:
...
@pytest.mark.parametrize(
    ...
    pytest.param([False, 5, 5, 2, 3, 0, 0, 15, 15, True, 3], id="consistent_yield:T|include_other_category:F"),
...
(
    ...
    pytest.param([False, 5, 5, 2, 3, 0, 0, 15, 15, True, 3], id="consistent_yield:T|include_other_category:F"),
    pytest.param([False, 5, 5, 2, 3, 0, 0, 15, 15, False, 3], id="consistent_yield:F|include_other_category:F"),
    pytest.param([False, 5, 5, 5, 5, 5, 5, 15, 15, True, 5],marks=pytest.mark.xfail(
                reason="AssertionError: Invalid input: If trust_labelling=True in orderly.extract, then num_cat and num_reag must be 0. If trust_labelling=False, then num_agent must be 0."), id="consistent_yield:T|include_other_category:F|fives"),
),
indirect=True
)
...
def test_frequency(cleaned_df_params):
    ...
    min_frequency_of_occurance_secondary,
    include_other_category,
    map_rare_to_other_threshold) = params
