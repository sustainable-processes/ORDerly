...
class Cleaner:
    ...
    extracted_ords_path: pathlib.Path
    ...
    def _merge_extracted_ords(self) -> pd.DataFrame:
        ...
        onlyfiles = [
            f
            for f in os.listdir(self.extracted_ords_path)
            if os.path.isfile(os.path.join(self.extracted_ords_path, f))
        ]

        dfs = []
        with tqdm.contrib.logging.logging_redirect_tqdm(loggers=[LOG]):
            for file in tqdm.tqdm(onlyfiles, disable=self.disable_tqdm):
                if file[0] != ".":  # We don't want to try read .DS_Store
                    filepath = self.extracted_ords_path / file
...
@click.option(
    "--extracted_ords_path",
    default="data/orderly/extracted_ord_data",
    type=str,
...
def main_click(
    ...
    main(
        ...
        extracted_ords_path=pathlib.Path(extracted_ords_path),
...
def main(
    ...
    if not isinstance(extracted_ords_path, pathlib.Path):
        raise ValueError(f"Expect pathlib.Path: got {type(extracted_ords_path)}")
    ...
    molecules_to_remove = pd.read_parquet(
        molecules_to_remove_path
