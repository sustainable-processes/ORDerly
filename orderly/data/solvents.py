...
def get_solvents_dict(
    ...
    def get_df(name: str, solvents_df: typing.Optional[pd.DataFrame] = None, ) -> pd.DataFrame:
        if solvents_df is None:
            solvents_df = orderly.data.get_solvents()
        else:
            solvents_df = solvents_df.copy()
        df = solvents_df[[name, "canonical_smiles"]].dropna().rename({name: "identifer"}, axis=1)
        df['identifer'] = df['identifer'].str.lower()
        return df

    return pd.concat([
        get_df(name=i, solvents_df=solvents) for i in ["stenutz_name", "cosmo_name", "other_name_1", "other_name_2"]
    ],axis=0).set_index("identifer").to_dict()['canonical_smiles']
