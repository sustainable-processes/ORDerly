...
class Cleaner:
    ...
    def __init__(
        ...
        save_with_label_called_other: bool,
        disable_tqdm: bool = False,
    ):
        """

        Args:
            clean_data_file_name (str): [description]
            ...
            include_other_category (bool): Will save reactions with infrequent molecules (below min_frequency_of_occurance_primary/secondary but above save_with_label_called_other)
                                           by mapping these molecules to the string 'other'
            save_with_label_called_other (bool): Frequency cutoff (see above).
    ...
    def merge_pickles(self) -> pd.DataFrame:
        """
        ...
        folder_path = pathlib.Path("data/USPTO/pickled_data/")
        onlyfiles = [
            f
    ...
    def get_number_of_columns_to_keep(
    ...
    def remove_reactions_with_too_many_of_component(
        self, df: pd.DataFrame, component_name: str
    ):
        number_of_columns_to_keep = self.get_number_of_columns_to_keep()[component_name]
        ...
        if component_name == "reactant":
            ...
            raise KeyError(
                "component_name must be one of: reactant, product, yield, solvent, agent, catalyst, reagent"
            )
        ...
        while pre_len > post_len:
            ...
            if self.include_other_category:
                ...
                set_to_other = value_counts[
                    (value_counts >= self.save_with_label_called_other)
