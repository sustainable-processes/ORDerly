...
Taken from https://github.com/mlcommons/algorithmic-efficiency which is
...
def _generate_dim(
    ...
    for i in range(1, num_samples + 1):
        ...
        while i:
            ...
            i //= base
        if per_dim_shift:
            num = math.fmod(num + dim_shift, 1.0)  # type: ignore
...
def generate_sequence(
    ...
    shuffled_seed_sequence: Optional[Matrix] = None,
) -> Matrix:
    ...
    if shuffled_seed_sequence is not None:
        ...
        for d in range(num_dims):
            if len(shuffled_seed_sequence[d]) != primes[d]:  # type: ignore
                raise ValueError(
    ...
    for d in range(num_dims):
        ...
        dim_sequence = _generate_dim(
            ...
            base=primes[d],
            shuffled_seed_sequence=dim_shuffled_seed_sequence,  # type: ignore
...
def _generate_double_point(
    ...
    if coerce_to_int:
        rescaled_value = int(rescaled_value)
...
def loguniform(
    name: Text, range_endpoints: Tuple[float, float], coerce_to_int: bool
) -> _GeneratorFn:
...
def uniform(
    name: Text,
    search_points: Union[_DiscretePoints, Tuple[float, float]],
    coerce_to_int: bool,
) -> _GeneratorFn:
...
def sweep(name, feasible_points: Sequence[Any]) -> _SweepSequence:
    return [{name: x} for x in feasible_points.feasible_points]  # type: ignore
    ...
    for trial_index in range(length):
        ...
        for hyperparameter_index in range(len(generator_fns_or_sweeps)):
            ...
            if callable(generator_fns_or_sweeps[hyperparameter_index]):
                ...
                hyperparameter_name, value = generator_fn(halton_point)  # type: ignore
            else:
                sweep_list = generator_fns_or_sweeps[hyperparameter_index]
                if trial_index > len(sweep_list):  # type: ignore
                    break
                hyperparameter_point = sweep_list[trial_index]  # type: ignore
...
def generate_search(search_space: _DictSearchSpace, num_trials: int) -> List[Dict]:
    ...
    for name, space in search_space.items():
        if "feasible_points" in space:  # Discrete search space.
            generator_fn = uniform(
                name, discrete(space["feasible_points"]), coerce_to_int=False  # type: ignore
            )
        else:  # Continuous space.
            coerce_to_int = True if space["type"] == "integer" else False
            if space["scaling"] == "log":
                generator_fn = loguniform(
                    name,
                    interval(space["min"], space["max"]),  # type: ignore
                    coerce_to_int=coerce_to_int,
                )
            else:
                generator_fn = uniform(
